...
```{python}
# Create DAG for 1747 understanding
pgm = daft.PGM([4,2], origin=[0,0], dpi=150, alternate_style="outer")
...
pgm.render()
plt.title("1747 Understanding: Lemons Prevent Scurvy", fontsize=14)
plt.show()
```
...
```{python}
# Create DAG for misguided belief
pgm = daft.PGM([6,2], origin=[0,0], dpi=150, alternate_style="outer")
...
pgm.render()
plt.title("Misguided Belief: Acid Kills Bacteria Preventing Scurvy", fontsize=14)
plt.show()
```
...
```{python}
# Create DAG for correct understanding
pgm = daft.PGM([5,2], origin=[0,0], dpi=150, alternate_style="outer")
...
pgm.render()
plt.title("1928 Discovery: Vitamin C Prevents Scurvy", fontsize=14)
plt.show()
```
